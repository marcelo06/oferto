...
class Slides extends ModelBase
{
	...
	function __construct($token = null){
		parent::__construct();
		$this->dirfile = FILES."slides/";
		$this->dirfileout = URLFILES."slides/";
        $this->token = $token;
	...
	public function guardar ($id=0 ){
		if(is_numeric($id) && $id > 0){
			$this->db->update($this->datos ,SLIDES, "id_galeria = '$id'" );
		}else{
			$this->db->insert($this->datos, SLIDES);
	...
	private function guardarDocs($id){
		if($this->nf[$this->token] > 0){
			$this->db->query("delete from ".SLIDE_IMAGENES." where id_galeria = '$id'");
			for($j=1;$j<=$this->nf[$this->token];$j++)
				if($this->docs[$this->token]['estado'][$j] != 0){
					...
					$this->db->insert($gal, SLIDE_IMAGENES);
				}
		}
	}
	...
	public function borrar($id){
		$outimg = $this->db->query("select archivo from ".SLIDE_IMAGENES." where id_galeria = '$id' ");

		if($this->db->num_rows($this->db->query("select id_galeria from ".SLIDES." where id_galeria = '$id' ")) > 0 ){
			...
			$this->db->query("delete from ".SLIDES." where id_galeria = '$id' ");
			return true;
		}else
			return false;
	}
	...
	public function obtener($id){
		$this->datos = $this->db->fetch_array($this->db->query("select * from ".SLIDES." where id_galeria = '$id'"));
		$this->obtenerDocs($id);
		return $this->datos;
	}
	
	public function obtenerEmpresa($id){
		$this->datos = $this->db->fetch_array($this->db->query("select * from ".SLIDES." where id_empresa = '$id'"));
		$this->obtenerDocs($id);
		return $this->datos;
	}
	...
	public function obtener_img(){
		return $this->db->result_array($this->db->query("select i.* from ".SLIDE_IMAGENES." i left join ".SLIDES." s on s.id_galeria=i.id_galeria where s.id_empresa =".$_SESSION['id_empresa']));
	...
	private function obtenerDocs($id){
		...
		$out = $this->db->query("select * from ".SLIDE_IMAGENES." where id_galeria = '$id' order by posicion");
	...
	public function obtenerDoc($id,$tipo='s'){
		$img = $this->db->fetch_object($this->db->query("select * from ".SLIDE_IMAGENES."  where id_galeria = '$id' order by posicion asc"));
	...
	public function agregarDoc( $campo, $referencia, $vinculo,$idDiv ){
		...
		$img->image_background_color='#FAFAFA';
	...
	public function obtenerGaleria($id_galeria){
		$this->datos = $this->db->fetch_array($this->db->query("select * from ".SLIDES." where id_galeria = '$id_galeria'"));
		return $this->datos;
	}
	...
	public function listarGaleria($id_galeria){
	    $out = $this->db->result_array($this->db->query("select * from ".SLIDE_IMAGENES." where id_galeria = '$id_galeria' order by posicion asc "));
		$posicion = 0;
		foreach($out as $reg){
			$this->nf[$this->token]++;
			$posicion++;
