( function ( $ ) {
	...
	$.widget( 'ui.placepicker', {
		_init: function () {
			...
			if ( options.initialLocation ) {
				if ( !self.isCompleteLocation( options.initialLocation ) ) {
					throw 'Localidad inicial no esta completa.';
				}
		...
		_loadMapEngine: function () {
			...
			if ( engine === undefined ) {
				...
				if ( !engine ) {
					throw 'No hay mapas registrados.'
				}
			}
	...
	$.extend( $.ui.placepicker, {
		defaults: {
			...
			resultMarker: {
				icon: 'system/src/placepicker/images/result.png'
			},
			formSelectors: {
				street: '[name=street]',
				city: '[name=city]',
	...
	$.ui.placepicker.regional[''] = {
		locationText: 'Localidad:',
		noLocationText: 'Ciudad, Provincia, PaÃ­s',
		noticeIncompleteText: 'La consulta regreso un resultado incompleto, intente de nuevo.',
		resultText: 'Es este el lugar correcto?',
		cancelText: 'Cancelar',
		searchText: 'Buscar',
		saveText: 'Aceptar'
	};
