...
class Banner extends ControlBase {
	...
	public function editar(){
		...
		if($this->input->post('id')){
			...
			$reg['id_banner'] = $id;
			$reg['mensaje'] = "Galeria guardada";
		}
		else{
			 $gal->iniciar();
			}
		$this->view->show("admin/edit_banner.php", $reg);
	  }

	public function cargar_galeria(){
		$gal = new Banners($this->input->post('token'));
	  ...
	  '<li class="ui-state-default addimage imageli'.$i.'"  value="'.$i.'" id="'.$i.'">
		<div class="extras">
		  ...
		  <a class="fancybox-thumbs" href="'.$gal->dirfileout.'b'.$reg['archivo'].'"  data-fancybox-group="thumbs" title= "'.$reg['referencia'].'" ></a><img class="imggal" id="addClass'.$i.'" src="'.$gal->dirfileout.'s'.$reg['archivo'].'"  width="100%" />
		  ...
		  <div class="editimg" ><a class="form_referencia" href="banner-form_referencia-id_doc-'.$i.'-cod-'.$this->input->post('token').'"><img src="'.URLVISTA.'admin/img/icon-02.png"></a></div>
	   </li>';
	 ...
	 while($i <= $gal->nf[$gal->token]){
	   if($gal->docs[$gal->token]['estado'][$i] != 0){
	      ...
	      <div class="prev" id="prev'.$gal->docs[$gal->token]['idDiv'][$i].'">
	      	<a  style="cursor:pointer" class="screenshot" rel="'.$gal->dirfileout.'s'.$gal->docs[$gal->token]['archivo'][$i].'"><img src="'.URLBASE.'system/src/uploadify/lupita.png" width="90" height="40" border="0" /></a>
	      </div>
	      <div class="cancel">
   ...
   /**
	...
	public function obtener_archivo(){
		$galeria = new Banners($this->input->post('token'));
		if( $this->input->post('id_doc') ){
