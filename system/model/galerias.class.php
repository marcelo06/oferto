...
class Galerias extends ModelBase
{
	...
	function __construct($token = null){
		parent::__construct();
		$this->dirfile = FILES."galerias/";
		$this->dirfileout = URLFILES."galerias/";
        $this->token = $token;
	...
	public function guardar ($id=0 ){
		if(is_numeric($id) && $id > 0){
			$this->db->update($this->datos ,GALERIAS, "id_galeria = '$id'" );
		}else{
			$this->db->insert($this->datos, GALERIAS);
	...
	private function guardarDocs($id){
		if($this->nf[$this->token] > 0){
			$this->db->query("delete from ".GALERIA_IMAGENES." where id_galeria = '$id'");
			for($j=1;$j<=$this->nf[$this->token];$j++)
				if($this->docs[$this->token]['estado'][$j] != 0){
					...
					$this->db->insert($gal, GALERIA_IMAGENES);
				}
		}
	}
	...
	public function borrar($id){
		$outimg = $this->db->query("select archivo from ".GALERIA_IMAGENES." where id_galeria = '$id' ");

		if($this->db->num_rows($this->db->query("select id_galeria from ".GALERIAS." where id_galeria = '$id' ")) > 0 ){
			if($this->db->num_rows($outimg) > 0 ){
				while($delimg = $this->db->fetch_object($outimg)){
					...
					if( file_exists($this->dirfile."m".$delimg->archivo) )
					   @unlink($this->dirfile."m".$delimg->archivo);

					if( file_exists($this->dirfile."a".$delimg->archivo) )
					   @unlink($this->dirfile."a".$delimg->archivo);

				
				}
			}
			$this->db->query("delete from ".GALERIAS." where id_galeria = '$id' ");
			return true;
		}else
			return false;
	}
	...
	public function obtener($id){
		$this->datos = $this->db->fetch_array($this->db->query("select * from ".GALERIAS." where id_galeria = '$id'"));
		$this->obtenerDocs($id);
		return $this->datos;
	}
	...
	public function obtener_img($id){
		return $this->db->result_array($this->db->query("select * from ".GALERIA_IMAGENES." where id_galeria = '$id'"));
	...
	private function obtenerDocs($id){
		...
		$out = $this->db->query("select * from ".GALERIA_IMAGENES." where id_galeria = '$id' order by posicion");
	...
	public function obtenerDoc($id,$tipo='s'){
		$img = $this->db->fetch_object($this->db->query("select * from ".GALERIA_IMAGENES."  where id_galeria = '$id' order by posicion asc"));
	...
	public function agregarDoc( $campo, $referencia, $idDiv ){
		$this->nf[$this->token]++;
		...
		if($this->nf[$this->token] >0 )
			for($i=1;$i<=$this->nf[$this->token];$i++){
				if($this->docs[$this->token]['estado'][$i] == $tipo){
                   ...
                   if( file_exists($this->dirfile."m".$this->docs[$this->token]['archivo'][$i]) )
                    @unlink($this->dirfile."m".$this->docs[$this->token]['archivo'][$i]);
				}
			}
	}
	...
	public function obtenerGaleria($id_galeria){
		$this->datos = $this->db->fetch_array($this->db->query("select * from ".GALERIAS." where id_galeria = '$id_bien'"));
		return $this->datos;
	}
	...
	public function iniciar(){
		$_SESSION['nf']=array();
		$_SESSION['docs'][$this->token] = array();
	...
	public function listarGaleria($id_galeria){
	    $out = $this->db->result_array($this->db->query("select * from ".GALERIA_IMAGENES." where id_galeria = '$id_galeria' order by posicion asc "));
		$posicion = 0;
		foreach($out as $reg){
			$this->nf[$this->token]++;
			$posicion++;
