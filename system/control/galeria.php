...
class Galeria extends ControlBase {

	public function cargar_galeria(){
		$gal = new Galerias($this->input->post('token'));
	  ...
	  '<li class="ui-state-default addimage imageli'.$i.'"  value="'.$i.'" id="'.$i.'">
		  ...
		  <a class="fancybox-thumbs" href="'.$gal->dirfileout.'b'.$reg['archivo'].'"  data-fancybox-group="thumbs" title= "'.$reg['referencia'].'" ></a><img class="imggal" id="addClass'.$i.'" src="'.$gal->dirfileout.'s'.$reg['archivo'].'"  width="104" height="56" />
		  ...
		  <div class="editimg" ><a class="form_referencia" href="galeria-form_referencia-id_doc-'.$i.'-cod-'.$this->input->post('token').'"><img src="'.URLVISTA.'admin/img/icon-02.png"></a></div>
	   </li>';
	 ...
	 while($i <= $gal->nf[$gal->token]){
	   if($gal->docs[$gal->token]['estado'][$i] != 0){
	      ...
	      <div class="prev" id="prev'.$gal->docs[$gal->token]['idDiv'][$i].'">
	      	<a  style="cursor:pointer" class="screenshot" rel="'.$gal->dirfileout.'s'.$gal->docs[$gal->token]['archivo'][$i].'"><img src="'.URLBASE.'system/src/uploadify/lupita.png" width="104" height="40" border="0" /></a>
	      </div>
	      <div class="cancel">
   ...
   /**
	...
	public function obtener_archivo(){
		$galeria = new Galerias($this->input->post('token'));
		if( $this->input->post('id_doc') ){
	...
	public function form_referencia(){
       ...
       if($this->input->post('id_doc') && $this->input->post('token')){
	     ...
	     $reg['mensaje'] = 'Descripci&oacute;n actualizada.<br> Use el botón guardar para confirmar los cambios';
	       ...
	       $reg['referencia']   = $_SESSION['docs'][$this->get['cod']]['referencia'][$this->get['id_doc']] ;

	   }
	   $this->view->show('admin/edit_referencia.php', $reg);
	}
