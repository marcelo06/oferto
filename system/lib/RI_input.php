...
class RI_input {
	...
	*/
	...
	function getVariables($url)
	{
		...
		if($cantPartes == 1){
			$vars['control'] = $partes[0];
		}
		
		if($cantPartes >= 2){
			$vars['control'] = $partes[0];
			$vars['accion'] = $partes[1];
		}
		
		if($cantPartes > 2 && $cantPartes % 2 != 0 )
			return false;
		
		for($c = 2; $c < $cantPartes; $c = $c + 2)
		{
	 	...
	 	$vars = $this->_clean_input_data($vars);
		return $vars;
	}
	...
	*/
	function _fetch_from_array(&$array, $index = '', $xss_clean = FALSE)
	{
		...
		if ($xss_clean === TRUE)
		{
			return $this->_clean_input_data($array[$index]);
		}
