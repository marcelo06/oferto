...
$(document).ready(function(){
    ...
    $('#file_upload').uploadifive({
            ...
            'queueID'       : 'fileQueue',
            'uploadScript'  : 'banner-subir_archivo',
            'buttonText'    : '',
            'fileType'     : 'image',
            onUploadComplete:function(file, data) {
              if(data){
                resp = eval('(' +data+ ')');
                $('#nuevoli').append('<div class="extras"></div><a class="fancybox-thumbs" title="" data-fancybox-group="thumbs" href="'+resp.archivo+'"></a><img class="imggal" id="addClass'+file.id+'" src="'+resp.thumbnail+'"  width="100%" /><div class="deleteimg" id="dele'+resp.pos+'" onclick="borrar('+resp.pos+')"><img src="'+urlvista+'admin/img/Closed.png" title="eliminar imagen" alt="eliminar imagen" ></div><div class="editimg" ><a class="form_referencia" href="banner-form_referencia-id_doc-'+resp.pos+'-cod-'+$('#token').val()+'"><img src="'+urlvista+'admin/img/icon-02.png"></a></div>');
       ...
       $(function(){
        $( ".sortable" ).sortable({
          stop: function(evento, ui){
                  ...
                  $.post('banner-ordenar',{arreglo : result, 'token' : $('#token').val()  });
